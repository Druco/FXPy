...
extern FXuint fxrandom(FXuint& seed);
...
/// Duplicate memory
extern FXint fxmemdup(void** ptr,const void* src,unsigned long size);

/// Free memory, resets ptr to NULL afterward
extern void fxfree(void** ptr);

/// Error routine
extern void fxerror(const char* format,...) FX_PRINTF(1,2) ;

/// Warning routine
extern void fxwarning(const char* format,...) FX_PRINTF(1,2) ;

/// Log message to [typically] stderr
extern void fxmessage(const char* format,...) FX_PRINTF(1,2) ;
...
extern void fxtrace(unsigned int level,const char* format,...) FX_PRINTF(2,3) ;

/// Sleep n microseconds
extern void fxsleep(unsigned int n);

/// Match a file name with a pattern
extern bool fxfilematch(const char *pattern,const char *string,FXuint flags=(FILEMATCH_NOESCAPE|FILEMATCH_FILE_NAME));
...
/// Get process id
extern FXint fxgetpid();

/// Convert string of length len to MSDOS; return new string and new length
extern bool fxtoDOS(FXchar*& string,FXint& len);

/// Convert string of length len from MSDOS; return new string and new length
extern bool fxfromDOS(FXchar*& string,FXint& len);

/// Duplicate string
extern FXchar *fxstrdup(const FXchar* str);
...
extern FXchar* fxnamefromcolor(FXchar *colorname,FXColor color);
...
extern FXint fxieeefloatclass(FXfloat number);
extern FXint fxieeedoubleclass(FXdouble number);

/// Convert keysym to unicode character
extern FXwchar fxkeysym2ucs(FXwchar sym);

/// Convert unicode character to keysym
extern FXwchar fxucs2keysym(FXwchar ucs);

/// Parse geometry, a-la X11 geometry specification
extern FXint fxparsegeometry(const FXchar *string,FXint& x,FXint& y,FXint& w,FXint& h);
