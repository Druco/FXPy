...
public:
  ...
  */
  ...
  // Changed the return from FXint to int because there was a wierd interaction
  // between typemaps that caused a return statement to be inserted before the
  // code adding the other three output values
  %apply int *OUTPUT {FXint& GCP_INT, FXint& GCP_INT, FXuint& GCP_UINT};
  int getCursorPosition(FXint& GCP_INT,FXint& GCP_INT,FXuint& GCP_UINT) const;
