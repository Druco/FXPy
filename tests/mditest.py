...
class MDITestWindow(FXMainWindow):
    ...
    (ID_ABOUT,
     ID_NEW) = range(FXMainWindow.ID_LAST, FXMainWindow.ID_LAST+2)


    penguin_raw = "47 49 46 38 37 61 10 00 12 00 f2 00 00 b2 c0 dc \
  80 80 80 00 00 00 c0 c0 c0 10 10 10 ff ff ff e0 \
  a0 08 00 00 00 2c 00 00 00 00 10 00 12 00 00 03 \
  53 08 ba 21 12 2b c6 e6 9e 94 62 64 77 a3 20 4e \
  21 74 8b 60 9c 1a a9 98 a8 45 b2 85 38 76 4f 6c \
  bb 93 60 db 0d e4 d9 83 1d e7 57 18 04 6f b8 4c \
  ec 88 9c 01 0c 47 66 ac a2 38 19 76 36 83 c3 f0 \
  b4 5e 77 03 af f8 7b 13 77 ad d3 ad 75 61 a5 54 \
  02 27 45 02 00 3b"
...
\xe2\x96\x99\xe2\x96\x84\xe2\x96\x9f\n"
    ...
    unicode2 = "e2 80 be e2 80 be e2 80 be e2 80 be e2 80 be e2 80 be e2 80\
 be e2 80 be e2 80 be e2 80 be e2 80 be e2 80 be e2 80 be e2\
 80 be e2 80 be e2 80 be e2 80 be e2 80 be e2 80 be e2 80 be\
 e2 80 be e2 80 be e2 80 be e2 80 be e2 80 be e2 80 be e2 80\
 be e2 80 be e2 80 be e2 80 be e2 80 be e2 80 be e2 80 be e2\
 80 be e2 80 be e2 80 be 0a"

    def __init__(self, app):
        super().__init__(app, "MDI Widget Test",None,None,DECOR_ALL,0,0,800,600)
        # Map
        #_______________Message_Type____________ID___________________Message_Handler________
        ...
        self.app = app
        font = FXFont(self.app,"fixed,105,,,normal,,uni")
        ...
        # Icon for MDI Child
        penguin = bytes.fromhex(MDITestWindow.penguin_raw)
        self.mdiicon = FXGIFIcon(self.app,penguin)

        # Make MDI Menu
        mdimenu = FXMDIMenu(self,self.mdiclient)
        self.mdipopup = FXPopup(self)
        ...
        # not maximized.  To do self, they must have different ID's.
        FXMDIWindowButton(menubar,self.mdipopup,self.mdiclient,FXMDIClient.ID_MDI_MENUWINDOW,LAYOUT_LEFT)
        ...
        # Test window #1
        mdichild = FXMDIChild(self.mdiclient,"Child",self.mdiicon,self.mdipopup,MDI_NORMAL|MDI_TRACKING,10,10,400,300)
        scrollwindow = FXScrollWindow(mdichild,0)
        btn = FXButton(scrollwindow,MDITestWindow.tyger,None,None,0,LAYOUT_BOTTOM|LAYOUT_RIGHT)
        btn.setBackColor(FXRGB(255,255,255))
        btn.setFont(font)
        self.mdiclient.setActiveChild(mdichild)

        # Test window #2
        mdichild = FXMDIChild(self.mdiclient,"Child",self.mdiicon,self.mdipopup,MDI_NORMAL|MDI_TRACKING,20,20,400,300)
        scrollwindow = FXScrollWindow(mdichild,0)
        isostring = MDITestWindow.unicode.decode("utf-8")
        btn = FXButton(scrollwindow,isostring,None,None,0,JUSTIFY_LEFT|LAYOUT_CENTER_Y|LAYOUT_CENTER_X)
        btn.setFont(font)
        btn.setBackColor(FXRGB(255,255,255))

        # Test window #3
        mdichild = FXMDIChild(self.mdiclient,"Child",self.mdiicon,self.mdipopup,MDI_NORMAL|MDI_TRACKING,30,30,400,300)
        scrollwindow = FXScrollWindow(mdichild,0)
        btn = FXButton(scrollwindow,MDITestWindow.tyger,None,None,0,LAYOUT_FIX_WIDTH|LAYOUT_FIX_HEIGHT,0,0,600,1000)
        btn.setFont(font)
        btn.setBackColor(FXRGB(255,255,255))

        # File menu
        filemenu = FXMenuPane(self)
        FXMenuCommand(filemenu,"&New\tCtl-N\tCreate new document.",None,self,MDITestWindow.ID_NEW)
        FXMenuCommand(filemenu,"&Open\tCtl-O\tOpen document.")
        FXMenuCommand(filemenu,"&Quit\tCtl-Q\tQuit application.",None,self.app,FXApp.ID_QUIT,0)
        FXMenuTitle(menubar,"&File",None,filemenu)

        # Window menu
        windowmenu = FXMenuPane(self)
        FXMenuCommand(windowmenu,"Tile &Horizontally",None,self.mdiclient,FXMDIClient.ID_MDI_TILEHORIZONTAL)
        FXMenuCommand(windowmenu,"Tile &Vertically",None,self.mdiclient,FXMDIClient.ID_MDI_TILEVERTICAL)
        FXMenuCommand(windowmenu,"C&ascade",None,self.mdiclient,FXMDIClient.ID_MDI_CASCADE)
        FXMenuCommand(windowmenu,"&Close",None,self.mdiclient,FXMDIClient.ID_MDI_CLOSE)
        sep1 = FXMenuSeparator(windowmenu)
        sep1.setTarget(self.mdiclient)
        sep1.setSelector(FXMDIClient.ID_MDI_ANY)
        FXMenuCommand(windowmenu,FXString.null,None,self.mdiclient,FXMDIClient.ID_MDI_1)
        FXMenuCommand(windowmenu,FXString.null,None,self.mdiclient,FXMDIClient.ID_MDI_2)
        FXMenuCommand(windowmenu,FXString.null,None,self.mdiclient,FXMDIClient.ID_MDI_3)
        FXMenuCommand(windowmenu,FXString.null,None,self.mdiclient,FXMDIClient.ID_MDI_4)
        FXMenuCommand(windowmenu,"&Others...",None,self.mdiclient,FXMDIClient.ID_MDI_OVER_5)
        FXMenuTitle(menubar,"&Window",None,windowmenu)

        # Help menu
        helpmenu = FXMenuPane(self)
        FXMenuCommand(helpmenu,"&About FOX...",None,self,MDITestWindow.ID_ABOUT,0)
        FXMenuTitle(menubar,"&Help",None,helpmenu,LAYOUT_RIGHT)

    # About
    def onCmdAbout(self, sender, sel, ptr):
        FXMessageBox.information(self,MBOX_OK,"About MDI Test","Test of the FOX MDI Widgets\nWritten by Jeroen van der Zijp")
        return 1

    # New
    def onCmdNew(self, sender, sel, ptr):
        mdichild = FXMDIChild(self.mdiclient,"Child",self.mdiicon,self.mdipopup,MDI_NORMAL|MDI_TRACKING,20,20,300,200)
        scrollwindow = FXScrollWindow(mdichild,0)
        btn = FXButton(scrollwindow,MDITestWindow.tyger,None,None,0,LAYOUT_CENTER_X|LAYOUT_FILL_Y)
        btn.setBackColor(FXRGB(255,255,255))
        mdichild.create()
        return 1

# Make application
application = FXApp("MDIApp","FoxTest")

# Make window
win = MDITestWindow(application)
